-- Fix match_works RPC: add SET search_path = extensions, public
-- Required on self-hosted Supabase where pgvector is in the extensions schema.
-- The <=> operator is registered in extensions, not public, so without this
-- the function fails with "operator does not exist: extensions.vector <=> extensions.vector"
