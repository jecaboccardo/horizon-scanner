-- Migration: add canonical_work_id IS NULL filter to match_works + match_works_v2
-- Shadow (duplicate) papers are excluded from retrieval; only canonical versions surface.
-- They remain in the DB and are still findable by direct ID lookup.
...
COMMENT ON FUNCTION public.match_works_v2 IS
  '2026-05-22 adds canonical_work_id IS NULL filter — shadow (duplicate) papers excluded from retrieval.';
